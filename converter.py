...
class Converter:
    ...
    def baixarPlaylist(playlist_url, qualidade):
        ...
        if qualidade == 1:
            ...
            for video in playlist.videos:
                return video.streams.get_highest_resolution()  # Baixando arquivo MP4 na melhor qualidade possível.

    @staticmethod
    def converterArquivoUnico(playlist_url, qualidade):  # Função principal onde o arquivo é convertido.
        try:

            # Tratar arquivo e definir caminhos
            arquivo = Converter.baixarArquivoUrl(playlist_url, qualidade)
